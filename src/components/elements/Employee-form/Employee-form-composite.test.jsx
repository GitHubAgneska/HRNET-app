...
test('if text input does not pass validation on blur, then validator message should show', () => {
    
    const setValues = jest.fn('firstName');
    const { container, getByText } = render(<CompositeForm onClick={setValues} />)
    container.querySelector('input', { name: 'firstName' }).value = 'Le99er'


    // expect(getByText("firstName should only contain characters")).toBeInTheDocument();
